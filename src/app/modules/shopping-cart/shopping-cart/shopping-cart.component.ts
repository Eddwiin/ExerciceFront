...
export type DataShoppingCart = {
  bookGroup: BooksGroupByTitle[];
  reduction: number;
  finalPrice: number;
};
...
export class ShoppingCartComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.shoppingCart$ = this.store.select('shoppingCart').pipe(
      ...
      switchMap((bookGroup) => {
        ...
        this.totalPrice = _.sumBy(
          bookGroup.map((shop) => ({ price: shop.book.price })),
          'price'
        );
        const ids = bookGroup.map((shop) => shop.book.isbn);

        return this._shoppingCart.getReduction(ids).pipe(
          switchMap((reduction) => {
            return of({
              bookGroup,
              reduction,
              finalPrice: this.totalPrice - reduction,
            });
          })
        );
      })
  ...
  clearShoppingCart = () =>
    of<DataShoppingCart>({ bookGroup: [], reduction: 0, finalPrice: 0 });

  removeInShopCart(book: BookModel, qt: number): void {
    this.store.dispatch(
      new ShoppingCartActions.RemoveInShoppingCart(book.isbn)
    );
